    ...
    class Client
      ...
      def current_bucket
        @services[:current_bucket] ||= \
          Aliyun::OssDup::Struct::Bucket.new(name: bucket, client: self)
      end

      ClientService = ::Struct.new(:client)

      require 'aliyun/oss_dup/client/buckets'

      class BucketsService < ClientService
        include Client::Buckets
      end

      require 'aliyun/oss_dup/client/bucket_objects'

      class BucketObjectsService < ClientService
        include Client::BucketObjects
      end

      require 'aliyun/oss_dup/client/bucket_multiparts'
